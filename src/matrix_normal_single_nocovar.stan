...
data {
  ...
  int<lower=1> ntarget;               // number of levels of target var
  int<lower=1> nother;                // number of levels of non-target var
...
parameters {
  ...
  vector<lower=0>[ntarget] z_sigma_target;
  vector<lower=0>[nother] z_sigma_other;
...
transformed parameters {
  vector[nq] alpha;
  matrix[nq, N] mu;
  matrix[nq, N] eps_term;
  vector<lower=0>[ntarget] sigma_target;
  vector<lower=0>[nother] sigma_other;
  matrix[ntarget, ntarget] W_target;
  vector[nflat] mu_flat;
  vector[nflat] phi_flat;

  // define covariance matrix from correlations and standard deviations
  sigma_target = sigma_scale * z_sigma_target;
  sigma_other = sigma_scale * z_sigma_other;
  W_target = diag_pre_multiply(sigma_target, L_target);
  ...
  for (i in 1:N)
    eps_term[, i] = to_vector((W_target * diag_post_multiply(eps[i], sigma_other)));
...
model {
  ...
  z_sigma_target ~ std_normal();
  z_sigma_other ~ std_normal();
  
  // prior for correlation matrix
  L_target ~ lkj_corr_cholesky(1.);
