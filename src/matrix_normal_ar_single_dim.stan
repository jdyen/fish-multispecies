functions {
  ...
  //          nclass dimension
  real matrix_normal_single_cholesky_lpdf(matrix y, matrix mu, matrix U_chol, vector sigma) {
    ...
    vector[m] sigma_inv = rep_vector(1.0, m) ./ square(sigma);
    ...
    matrix[m, n] Q = diag_pre_multiply(sigma_inv, y_resid'); // inverse(V) * y_resid' for diagonal V

    real numerator = trace(Q * P);
  
    // calculate normalisation term (denominator of log pdf)
    real log2pi = 1.8378770664093453;
    real denom = n * m * log2pi + 2 * n * sum(diagonal(U_chol)) + 2 * m * sum(sigma);
...
model {
  ...
  for (i in 1:N) {
    for (t in 1:ntime) {
      log_lambda[i, t] ~ matrix_normal_single_cholesky(mu[i, t], W_sp, sigma_class);
